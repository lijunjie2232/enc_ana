...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.intervals = configs.intervals
        self.stocks = configs.stocks
        self.pred_scale = configs.pred_scale
        # Embedding
        assert configs.d_model % 16 == 0, "d_model must be divisible by 8"
        self.enc_embedding = DataEmbedding_inverted(
            configs.seq_len,
            configs.d_model // 16 * 13,
            configs.embed,
            configs.freq,
            configs.dropout,
        )
        ...
        self.time_embedding = [
            nn.Embedding(
                configs.seq_len // scale,
                configs.d_model // 16,
            )
            for scale in self.pred_scale
        ]  # 对应1h/4h/1d的预测密度
        self.class_strategy = configs.class_strategy
        # Encoder-only architecture
        assert (
            configs.d_model * configs.expand // configs.head_dim // 2 >= 1
        ), "d_model must be divisible by head_dim"
        ...
        self.projector = nn.ModuleList(
            [
                nn.Linear(configs.d_model, configs.pred_len // scale)
                for scale in self.pred_scale  # 对应1h/4h/1d的预测密度
            ]
        )
        # self.scale_adapter = nn.Linear(sum([configs.pred_len//s for s in [1,4,24]]), configs.pred_len)
